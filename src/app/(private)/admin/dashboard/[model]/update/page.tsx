...
function useDeleteMutation(model: string, router: any) {
  const universalDelete = useDeleteModel(model, {
    onSuccess: () => router.push(`/admin/dashboard/${model}`),
    showToast: true
  })
...
export default function UpdateModelPage({ params }: PageProps) {
    ...
    return (
      <div className="space-y-6">
        <Card>
          ...
          <CardContent>
            <Button onClick={() => router.push(`/admin/dashboard/${model}`)} variant="outline">
              Back to {model} list
            </Button>
          </CardContent>
        </Card> 
  ...
  const onSubmit = (formData: any) => {
    ...
    updateMutation.mutate({ id, data: transformedData }, {
      onSuccess: () => {
        // Если меняли ID кошелька, уводим на список, чтобы не оставаться на старом ID
        if ((model === 'appWalletUsdt' || model === 'appWalletUsdc') && (formData.id && formData.id !== id)) {
          router.push(`/admin/dashboard/${model}`)
        }
      }
    })
  }
  ...
  const transformFormData = (model: string, data: any) => {
    ...
    switch (model) {
      case 'users': {
        ...
        if (out.usdtWalletId === '' || out.usdtWalletId === false) out.usdtWalletId = 'null'
        if (out.usdcWalletId === '' || out.usdcWalletId === false) out.usdcWalletId = 'null'
        return out
      }
      case 'appWalletUsdt':
      case 'appWalletUsdc': {
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <Button 
              variant="outline" 
              size="sm"
              onClick={() => router.push(`/admin/dashboard/${model}`)}
            >
            ...
            <div className="flex items-center gap-4 pt-6 border-t">
              ...
              {model === 'users' && (data as any)?.inviterId && (
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => router.push(`/admin/dashboard/users/update?id=${(data as any).inviterId}`)}
                  disabled={updateMutation.isPending || deleteMutation.isPending}
                >
                  Go to inviter
                </Button>
