...
const modelConfigs: Record<string, ModelConfig> = {
  admins: {
    title: "Edit admin",
    description: "Edit existing admin account",
    icon: "🛡️",
    fields: [
      { key: "username", label: "Username", type: "text", required: true },
      { key: "password", label: "New password", type: "text", required: false },
      { key: "isSuperAdmin", label: "Super admin privileges", type: "checkbox" }
    ]
  },
  notifications: {
...
function useUpdateMutation(model: string, router: any, currentId?: string) {
  ...
  const onSuccessWithCacheInvalidation = (data: any, variables?: any) => {
    ...
    // Переходим на список
    router.push(`/superadmin/dashboard/${model}`)
  }
  ...
  switch (model) {
    ...
    case 'users':
      return userUpdate
    case 'admins':
      return universalUpdate // Используем универсальный хук для админов
    default:
...
function useDeleteMutation(model: string, router: any) {
  ...
  switch (model) {  
    ...
    case 'users':
      return userDelete
    case 'admins':
      return universalDelete // Используем универсальный хук для админов
    default:
...
export default function UpdateModelPage({ params }: PageProps) {
  ...
  const { register, handleSubmit, formState: { isSubmitting, errors }, reset, control } = useForm()
    ...
    return (
      <div className="space-y-6">
        <Card>
          ...
          <CardContent>
            <Button onClick={() => router.push(`/superadmin/dashboard/${model}`)} variant="outline">
              Back to {model} list
            </Button>
          </CardContent>
        </Card>
  ...
  const transformFormData = (model: string, data: any) => {
    ...
    switch (model) {
      case 'admins': {
        const out: any = { ...data }
        // Не отправляем пустой пароль или undefined
        if (out.password === '' || !out.password || out.password === undefined) {
          delete out.password
        }
                    ...
                    {(field.type === "text" || field.type === "email") && (
                      <div className="space-y-1">
                        <Input
                          ...
                          placeholder={field.key === "password" ? "Leave empty to keep current password" : `Enter ${field.label.toLowerCase()}`}
            ...
            <div className="flex items-center gap-4 pt-6 border-t">
              ...
              {model === 'users' && (data as any)?.inviterId && (
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => router.push(`/superadmin/dashboard/users/update?id=${(data as any).inviterId}`)}
                  disabled={updateMutation.isPending || deleteMutation.isPending}
                >
                  Go to inviter
                </Button>
