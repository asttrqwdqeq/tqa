...
// Хук для получения упрощенных данных пользователей
function useSimpleUsersData(paginationParams: { page: number; limit: number }, status?: string) {
  ...
  if (error) {
    console.warn('API error for simple users:', error)
    return { 
      isLoading: false, 
      error: null,
      data: [],
  ...
  if (apiData) {
    console.log('📊 Simple users data:', apiData)
...
function useModelData(model: string, paginationParams: { page: number; limit: number }) {
  ...
  const { data: apiData, isLoading, error } = useModelList(model, paginationParams, {
    enabled: ['notifications', 'appWallet', 'users', 'leaderboardParticipants', 'admins'].includes(model),
    staleTime: 1000 * 60 * 2, // 2 минуты кэширование
  })
  ...
  if (apiData) {
    ...
    // Специальная обработка для admins - бэкенд возвращает массив напрямую
    if (model === 'admins' && Array.isArray(apiData)) {
      return {
        data: apiData,
...
function getTableConfig(
  ...
  const baseActions = [
    {
      ...
      onClick: (row: any) => {
        router.push(`/superadmin/dashboard/${model}/update?id=${row.id}`)
      },
  ...
  switch (model) {
    ...
    case "users":
      // Для активных пользователей показываем упрощенную таблицу
      if (isSimpleMode) {
        return {
          columns: [
          ...
          actions: [
            ...baseActions,
            {
              ...
              onClick: (row: any) => {
                window.location.href = `/superadmin/dashboard/user-moderation?userId=${row.id}&action=username`
              },
              variant: "secondary" as const
            },
            {
              label: "Edit Telegram ID",
              icon: <User className="h-4 w-4" />,
              onClick: (row: any) => {
                window.location.href = `/superadmin/dashboard/user-moderation?userId=${row.id}&action=tgid`
              },
              variant: "secondary" as const
            }
          ]
      ...
      // Полная таблица для всех пользователей
      return {
        columns: [
          columnHelpers.text("tgId", "Telegram ID", { 
            width: 150,
      ...
      case "admins":
        return {
          columns: [
            ...
            columnHelpers.custom("isSuperAdmin", "Role", (value: boolean, row: AdminEntity) => (
            ...
            columnHelpers.date("createdAt", "Created", { width: 150 }),
            columnHelpers.date("updatedAt", "Last Updated", { 
              width: 150,
              render: (value: string | undefined) => {
                if (!value) return <span className="text-gray-400">Never</span>
          ...
          actions: [
            ...baseActions,
            {
              label: "Change Role",
              icon: <Settings className="h-4 w-4" />,
              onClick: (row: AdminEntity) => {
                const newRole = confirm(`Make ${row.username} a ${row.isSuperAdmin ? 'regular admin' : 'super admin'}?`)
                if (newRole) {
                  console.log('Toggle role for admin:', row.id, !row.isSuperAdmin)
                  alert(`Role will be changed to ${row.isSuperAdmin ? 'Admin' : 'Super Admin'}`)
                }
              },
              variant: "outline" as const
            }
...
export default function ModelPage({ params }: PageProps) {
  ...
  // Загружаем пользователей: активные или все
  const activeUsers = useSimpleUsersData(paginationParams, 'active')
  const allUsers = useModelData(model, paginationParams)
  
  const data = showActiveOnly ? activeUsers.data : allUsers.data
  const total = showActiveOnly ? activeUsers.total : allUsers.total
  const isLoading = showActiveOnly ? activeUsers.isLoading : allUsers.isLoading
  ...
  const tableConfig = getTableConfig(model, router, deleteMutation, prefetchItem, showActiveOnly)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight">
                {showActiveOnly && model === 'users' ? 'Active Users' : modelConfig.title}
              </h1>
              <p className="text-muted-foreground">
                {showActiveOnly && model === 'users' 
                  ? 'Manage active users with recent activity and deposits' 
        ...
        <div className="flex items-center gap-2">
          ...
          <Button 
            size="sm"
            onClick={() => router.push(`/superadmin/dashboard/${model}/create`)}
          >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {model === 'notifications' ? 'Important notifications' : 
               model === 'admins' ? 'Super Admins' : 'Active records'}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {model === 'notifications' 
                ...
                : model === 'admins'
                ? data?.filter((item: any) => item.isSuperAdmin === true).length || 0
                : data?.filter((item: any) => item.isActive !== false).length || 0
              }
            </div>
            <p className="text-xs text-muted-foreground">
              {model === 'notifications' ? 'Important notifications' : 
               model === 'admins' ? 'Super Admin accounts' : 'Active elements'}
            </p>
          </CardContent>
        </Card>
